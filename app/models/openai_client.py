...
Provide ONLY the clean movie title:"""
    ...
    def process_sms_message(self, message: str, system_prompt: str = None) -> Dict[str, Any]:
        """Process an SMS message using OpenAI."""
        if not self.client:
            logger.error("OpenAI API key not configured")
            return {"error": "OpenAI API key not configured", "original_message": message}
        
        logger.info(f"Processing SMS message with OpenAI: {message[:50]}...")
        
        # Use default system prompt if none provided
        if not system_prompt:
            system_prompt = "You are a helpful assistant responding to SMS messages. Keep responses concise and friendly."
        ...
        try:
            ...
            logger.info(f"OpenAI SMS response: '{response_text[:50]}...'")
            ...
            logger.error(f"OpenAI API error for SMS message '{message[:50]}...': {str(e)}")
            return {
                "error": f"OpenAI API error: {str(e)}",
                "original_message": message,
                "success": False
